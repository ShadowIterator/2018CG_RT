...
Vector3lf Camera::Emit( double i , double j ) {
...
void Camera::Outputfile(const std::string& file)
{
	std::cout << "Outputfile " << file << std::endl;
...
//			std::cout << i << " " << j << std::endl;
			byte* bs = &(timg.at<byte>(H - 1 - i, j * 3));
			bs[0] = (byte) (data[i][j].b * 255);
			bs[1] = (byte) (data[i][j].g * 255);
			bs[2] = (byte) (data[i][j].r * 255);
		}
